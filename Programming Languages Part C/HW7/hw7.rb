...
class GeometryValue 
  ...
  # default preprocess (except for linesegment)
  def preprocess_prog
    self
  end

  # evaluating value returns self
  def eval_prog env 
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg2
    if real_close(x1,x2)
      ...
      if real_close(order[0].y2, order[1].y1)
        Point.new(order[0].x2, order[0].y2)
      elsif order[0].y2 < order[1].y1
        NoPoints.new
      elsif order[0].y2 > order[1].y2
        LineSegment.new(order[1].x1,order[1].y1,order[1].x2,order[1].y2)
      else
        LineSegment.new(order[1].x1,order[1].y1,order[0].x2,order[0].y2)
      end
    else
      if x1 < seg2.x1
        order = [self, seg2]
      ...
      if real_close(order[0].x2, order[1].x1)
        Point.new(order[0].x2, order[0].y2)
      elsif order[0].x2 < order[1].x1
        NoPoints.new
      elsif order[0].x2 > order[1].x2
        order[1]
      else
        LineSegment.new(order[1].x1,order[1].y1,order[0].x2,order[0].y2)
      end

...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    cpad = []
    cdecmsg = "idk"
    cipher = ps4b.EncryptedMessage(ciphertext)
    for i in pads:
        score = 0
        decmsg = ciphertext.decrypt_message(i).get_text()
        for j in decmsg.split():
            if is_word(load_words(WORDLIST_FILENAME), j):
                score += 1
        if score >= hscore:
            hscore = score
            cpad = i
            cdecmsg = decmsg
    return ps4b.PlaintextMessage(cdecmsg, cpad)
...
def decode_story():
    ...
    '''
    cypher = ps4b.EncryptedMessage(get_story_string())
    msg = decrypt_message_try_pads(cypher, get_story_pads())
    return (msg.get_text())

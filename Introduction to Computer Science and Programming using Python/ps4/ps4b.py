...
class Message(object):
    def __init__(self, input_text):
        ...
        a Message object has one attribute:
            the message text
        '''
        self.text = input_text  # delete this line and replace with your code here
    ...
    def shift_char(self, char, shift):
        ...
        '''
        if not 0 <= abs(shift) < 94:
            shift = shift % 95
        shifted = ord(char) + shift # ascii val after shifted 
        if 32 <= shifted <= 126:
            return chr(shifted)
        elif shifted > 126:
            temp = shifted - 126
            return chr(32 + temp - 1)
        else:
            temp = 32 - shifted
            return chr(126 - temp + 1)
    ...
    def apply_pad(self, pad):
        ...
        for i in range(len(text)):
            word.append(self.shift_char(text[i], pad[i]))
        return ''.join(word)
...
class EncryptedMessage(Message):
    ...
    def decrypt_message(self, pad):
        ...
        for i in pad:
            npad.append(-i)
        return PlaintextMessage(self.apply_pad(npad), pad)

a = EncryptedMessage("05<Nvv`\\.8_0qvh?b?")

...
fun month_range(day1 : int, day2 : int) =
	...
	let
	    ...
	    val mn = what_month(day2);
	    fun gen_month_range(current_m : int) =
		if (current_m) <> mn
		then current_m :: gen_month_range(current_m + 1)
		else [current_m]
	in
	    gen_month_range(m1)
	end;
...
fun oldest(lod : (int * int * int) list) =
	...
	let
	    fun oldest_helper(lod : (int * int * int) list, oldest_so_far : (int * int * int)) =
		if null lod
		then oldest_so_far
		else
		    if is_older(oldest_so_far, hd lod)
		    then oldest_helper(tl lod, oldest_so_far)
		    else oldest_helper(tl lod, hd lod)
	in
	    SOME(oldest_helper(tl lod, hd lod))
	end;
